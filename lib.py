...
def write_line(line, first_note_written, last_delay, next_delay, press_delay, press_duration):
    ...
    if not first_note_written:
        ...
        delay_line = write_out_delay(last_delay)
    else:
        delay_line = write_out_delay(next_delay)
    line = write_out_one_beat(line, press_duration=press_duration)
    line_lines = [delay_line, line]
    return line_lines, first_note_written, press_delay


def update_delays_empty_line(first_note_written, last_delay, next_delay, between_beat_delay):
    if not first_note_written:
...
def write_out_bar_code(bar, press_duration, bpm, last_delay=0.0):
    ...
    if "1" not in bar.notes:
        ...
        return [], between_beat_delay * bar.num_rows + last_delay
    ...
    for idx, line in enumerate(bar.notes.split()):
        if "1" in line:
            ...
            last_delay, next_delay = update_delays_empty_line(first_note_written, last_delay, next_delay, between_beat_delay)

    assert(next_delay != 0.0)
    assert(next_delay >= (between_beat_delay - press_duration))
    return bar_lines, next_delay
